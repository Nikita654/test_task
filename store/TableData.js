...
export const mutations = {
    dataFilterDrop(state, name) {
        ...
        state.dataReqsList = state.dataReqsList.map((el) => {
            if (el[name.serverName] != name.name && name.name.indexOf('Все') == -1) {
                el.hide = true;
            } else {
                checkHideInReqsElements(el, state, name);
            }
            return el;
        })
    },
...
function checkHideInReqsElements(el, state, name) {
    if (el.hide == true) {
        let i = -1;
        state.filterList.forEach(dl => {
